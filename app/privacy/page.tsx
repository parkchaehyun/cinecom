    ...
    <div style={{ minHeight: "100dvh", background: "var(--page)", display: "flex", justifyContent: "center", padding: "20px 12px" }}>
      <main style={{ width: "100%", maxWidth: 680, background: "var(--card)", border: "1px solid var(--line)", borderRadius: "var(--r-xl)", boxShadow: "var(--shadow-card)", padding: "28px 22px 32px" }}>
        ...
        <p style={{ font: `500 var(--text-sm)/1.75 var(--font-sans)`, color: "var(--ink)", background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "var(--r-md)", padding: "14px 16px", margin: "0 0 26px", wordBreak: "keep-all" }}>
          ...
          제공받거나 저장하지 않습니다. 서비스가 저장하는 정보는 씨네꼼 카페에 <b>이미 공개된 예약글</b>의 제목과
          작성자 별명뿐이며, 이는 예약 현황을 보여주기 위한 것으로 상영일로부터 90일이 지나면 자동으로 삭제됩니다.
          원글이 카페에서 삭제되면 서비스의 사본도 즉시 삭제됩니다.
        </p>
        ...
        <S n="1" t="개인정보의 처리 목적">
          ...
          <ul>
            <li>
              <b>상영실 예약 현황 표시</b> — 씨네꼼 카페에 공개된 예약글을 수집·분석하여 대상영실·소상영실의 예약
              현황을 시간표로 보여줍니다.
            </li>
            <li>
              <b>예약글 작성</b> — 이용자 본인 명의로 씨네꼼 카페에 예약글을 작성합니다. 네이버 로그인은 오직 이
              권한을 얻기 위해서만 사용합니다.
            </li>
          </ul>
        </S>
        ...
        <S n="2" t="처리하는 개인정보의 항목">
          <h3>가. 네이버 로그인</h3>
          <p>
            서비스는 네이버 로그인에서 <b>이용자 식별자 외의 제공 정보(회원이름, 이메일 주소, 별명, 프로필 사진, 성별,
            생일, 연령대, 출생연도, 휴대전화번호)를 일체 조회하지 않습니다.</b> 이용자 식별자는 네이버가 기본 정보로
            제공하나 서비스는 이를 저장하지 않습니다. 서비스에는 회원 계정이나 회원 데이터베이스가 존재하지 않습니다.
          </p>
          <p>
            네이버 로그인 시 요청하는 <b>&lsquo;카페&rsquo; 권한</b>은 개인정보 조회 권한이 아니라, 이용자가 직접 예약글을
            작성하기 위해 필요한 쓰기 권한입니다.
          </p>
          <h3>나. 씨네꼼 카페의 공개 게시글</h3>
          <p>
            서비스는 씨네꼼 카페(cafe.naver.com/cinecom)에 <b>이미 공개되어 있는 예약글</b>에서 다음 항목을 수집합니다.
            예약글이 아닌 게시글은 수집·저장하지 않습니다.
          </p>
          <ul>
            <li>게시글 번호, 게시판 이름, 게시글 제목, 작성 일시</li>
            <li>
              <b>작성자 별명</b>(카페 닉네임) — 예약 현황에서 &lsquo;누가 예약했는지&rsquo;를 표시하기 위함
            </li>
            <li>제목에서 추출한 상영실·날짜·시간·영화 제목 및 제목에 이름이 적힌 경우 그 이름</li>
          </ul>
        </S>
        ...
        <S n="3" t="개인정보의 처리 및 보유 기간">
          <ul>
            <li>
              <b>상영일로부터 90일</b>이 지난 예약글 정보는 자동으로 삭제됩니다.
            </li>
            <li>
              <b>원글이 카페에서 삭제된 경우</b>, 서비스가 이를 확인하는 즉시(최대 10분 이내) 해당 정보를 삭제합니다.
            </li>
        ...
        <S n="5" t="개인정보 처리업무의 위탁">
          ...
          <p>
            서비스가 처리하는 개인정보는 <b>대한민국 내에서만 저장·처리되며, 국외로 이전되지 않습니다.</b>
          </p>
        </S>
        ...
        <S n="6" t="정보주체의 권리·의무 및 행사방법">
          ...
          <ul>
            ...
            <li>
              서비스가 표시하는 정보는 카페의 원글에서 온 것이므로, <b>카페에서 해당 글을 수정하거나 삭제하면 서비스의
              정보도 함께 수정·삭제</b>됩니다. 이것이 가장 빠르고 확실한 방법입니다.
            </li>
            <li>그 밖의 요구는 아래 제10항의 연락처로 접수하며, 지체 없이 조치합니다.</li>
          </ul>
        </S>
        ...
        <S n="8" t="개인정보의 안전성 확보조치">
          <ul>
            <li>수집 항목의 최소화 — 예약글이 아닌 게시글, 네이버 제공 정보는 애초에 수집하지 않습니다.</li>
            <li>전송 구간 암호화(HTTPS) 및 데이터베이스 접근 권한 통제</li>
        ...
        <S n="9" t="개인정보 자동 수집 장치의 설치·운영 및 거부">
          <p>
            서비스는 예약 현황 조회에는 쿠키를 사용하지 않으며, <b>로그인 없이 모든 예약 현황을 볼 수 있습니다.</b>
            이용자가 네이버 로그인을 한 경우에 한하여, 로그인 상태 유지를 위한 세션 쿠키 1개를 저장합니다. 이 쿠키에는
            카페 글쓰기에 필요한 인증 토큰이 담기며 최대 30일 후 만료됩니다. 브라우저 설정에서 쿠키를 거부할 수 있으나,
            이 경우 예약글 작성 기능은 사용할 수 없습니다. 로그아웃하면 즉시 삭제됩니다.
          </p>
        </S>

        <S n="10" t="개인정보 보호책임자 및 열람청구">
          <p>
            개인정보 처리에 관한 업무를 총괄해서 책임지고, 개인정보 처리와 관련한 정보주체의 문의·불만처리·피해구제
            등에 관한 사항을 다음의 연락처로 접수·처리하고 있습니다.
          </p>
          <ul>
        ...
        <S n="11" t="권익침해 구제방법">
          <p>
            개인정보 침해로 인한 구제를 받기 위하여 아래 기관에 분쟁해결이나 상담 등을 신청할 수 있습니다.
          </p>
          <ul>
